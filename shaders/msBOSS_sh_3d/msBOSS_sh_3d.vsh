...
void main()
{
	...
	v_worldPosition = (gm_Matrices[MATRIX_WORLD] * objectSpacePos).xyz;
	...
	float lightAngleDifference = max(dot(worldNormal, lightDir), 0.25);
	
	v_vColour = vec4(min(lightAmbient * lightAngleDifference, vec4(1.)).rgb, 1.);
}